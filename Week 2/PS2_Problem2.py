...
The monthly payment must be a multiple of $10 and is the same for all months. Notice that it is possible for the balance to become negative using this payment scheme, which is okay. A summary of the required math is found below:
...
while balance > 0:
    for i in range (12):
        balance = balance - monPaymentRate + ((balance - monPaymentRate) * monthlyInterestRate)
    if (balance > 0):
        monPaymentRate += 10
        balance = tempBalance
    elif balance <= 0:
...
print("Lowest Payment: " + str(monPaymentRate))
